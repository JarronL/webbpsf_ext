...
def BOSZ_spectrum(Teff, metallicity, log_g, res=2000, interpolate=True, **kwargs):
    ...
    model_dir = conf.WEBBPSF_EXT_PATH + 'spectra/bosz_grids/'
...
class planets_sb12(object):
    ...
    _base_dir = conf.WEBBPSF_EXT_PATH + 'spectra/spiegel/'
...
def sp_accr(mmdot, rin=2, dist=10, truncated=False,
    ...
    base_dir = conf.WEBBPSF_EXT_PATH + 'spectra/spiegel/' if base_dir is None else base_dir
...
def jupiter_spec(dist=10, waveout='angstrom', fluxout='flam', base_dir=None):
    ...
    base_dir = conf.WEBBPSF_EXT_PATH + 'spectra/solar_system/' if base_dir is None else base_dir
...
def linder_table(file=None, **kwargs):
    ...
    if file is None:
        indir = os.path.join(conf.WEBBPSF_EXT_PATH, 'spectra/linder/isochrones/')
...
def linder_filter(table, filt, age, dist=10, cond_interp=True, cond_file=None, **kwargs):
    ...
    base_dir = conf.WEBBPSF_EXT_PATH + 'spectra/cond_models/'
